    ...
    return (
        <div className={`bg-gray-100 border border-gray-300 flex flex-col p-3 gap-2 ${props.className || ''}`}>
            <div className="flex justify-between items-center w-full whitespace-nowrap">
                <h1 className="text-xs font-medium text-[#242424]">[{props.title}]</h1>
                <p className="text-xs font-medium text-[#242424] uppercase">[FIG. {String(props.figure).padStart(2, '0')}]</p>
            </div>

            <div className={`bg-white w-full h-full border border-gray-300 relative ${showPattern ? 'overflow-hidden' : ''} ${showPattern ? 'p-6' : 'p-0'}`}>
                {showPattern && (
